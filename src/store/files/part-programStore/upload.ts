...
interface FileUploadStore {
    ...
    uploadPartProgramFile: (machineryId: string, partProgramId: string) => Promise<boolean>;
}
...
export const useUploadPartProgramFileStore = create<FileUploadStore>((set, get) => ({
    isModalOpen: false,
    currentSection: null,
    uploadProgress: 0,
    isUploading: false,
    ...
    uploadPartProgramFile: async (machineryId: string, partProgramId: string) => {
        ...
        try {
            ...
            const response = await fetch(`${BASE_URL}machinery/part-programs/${partProgramId}/files`, {
                method: "POST",
                body: formDataToSend,
                headers: {
