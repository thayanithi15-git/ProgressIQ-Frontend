...
interface ResetPasswordLinkState {
  ...
  sendResetLink: (email: string, turnstileToken: string) => Promise<boolean>;
  resetPassword: (token: string, password: string) => Promise<boolean>;
  extractEmailFromToken: (token: string) => string;
}

export const useResetPasswordLinkStore = create<ResetPasswordLinkState>((set, get) => ({
  isLoading: false,
  email: '',
  password: '',
  confirmPassword: '',
  ...
  sendResetLink: async (email, turnstileToken) => {
    ...
    try {
      const response = await api.post("/auth/request-password-reset", { email, token: turnstileToken });
